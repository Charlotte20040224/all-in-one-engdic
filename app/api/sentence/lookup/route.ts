...
重要規則：
- "english" 欄位必須是英文句子（拉丁字母、標點），不能填中文
- 如果使用者輸入中文，請先翻譯成自然流暢、道地的英文再填入 "english"
- 如果使用者輸入的已經是英文，直接放入 "english"
- "ipa" 是整句的 IPA 國際音標（用 / / 包起來，重音用 ˈ）。預設用美式發音
- "ipaUS" = 整句用美式發音的 IPA
- "ipaGB" = 整句用英式發音的 IPA
- ipaUS 跟 ipaGB 都要回。**只有在句中真的含有英美發音「明顯可聽出差別」的字時才填不同值**，否則兩邊填一樣即可
...
- "vocabulary" 是句中重要單字的陣列。每項格式為：
  {"english":"英文單字","ipa":"IPA 音標","meaning":"繁體中文意思"}
  太基本的詞（例如 I、you、is、the、a）可省略；若整句都很基本，vocabulary 填 []

所有中文內容請使用繁體中文（Traditional Chinese）。

只回覆 JSON，格式範例：
{"english":"I am learning English.","ipa":"/aɪ æm ˈlɜːrnɪŋ ˈɪŋɡlɪʃ/","ipaUS":"/aɪ æm ˈlɜːrnɪŋ ˈɪŋɡlɪʃ/","ipaGB":"/aɪ əm ˈlɜːnɪŋ ˈɪŋɡlɪʃ/","zh":"我正在學英文。","grammar":"這是陳述句（現在進行式）。主詞 I（我）+ be 動詞 am + 現在分詞 learning（正在學）+ 受詞 English（英文）。be + V-ing 表示正在進行的動作。","grammarPattern":"S + be + V-ing + O","vocabulary":[{"english":"learn","ipa":"/lɜːrn/","meaning":"學習"},{"english":"English","ipa":"/ˈɪŋɡlɪʃ/","meaning":"英文"}]}
...
- "vocabulary" 必須是物件陣列，不可以是字串`
  ...
  try {
    const message = await client.messages.create({
      ...
      max_tokens: 4096,
      system: '你是一個英文句子分析助手。所有中文必須使用繁體中文（Traditional Chinese）。ipa 欄位一律使用 IPA 國際音標（用 / / 包圍）。',
    ...
    console.log('[sentence-lookup] english:', result.english, '| vocab count:', result.vocabulary.length)
